...
```json
[
  { "name": "team-a-peak", "weights": { "c3": 3.0 },
    "tenants": ["team-a"], "profiles": ["squilla/auto"], "hours": [1, 10] },
...
| 维度 | 字段 | 省略时 |
|---|---|---|
| 租户 | `tenants`（`tenantId` 列表） | 命中所有租户 |
| profile | `profiles`（虚拟路由 id 列表） | 命中所有 profile |
| 时间段 | `hours`（**UTC** 的 `[start, end)`，可跨零点） | 全天生效 |

三者都满足才算命中；三者全省略就是一条全局规则。**第一条命中的规则生效**，所以配置顺序
即优先级——把窄规则写在前面，宽规则兜底写在后面。坏条目跳过并告警，不会让路由启动失败。

> 时间用 UTC 而不是本地时区：中央可能多实例、跨机房部署，用本地时区会让同一条规则在不同
> 实例上于不同时刻生效。要表达「北京时间 09:00–18:00」，写 `[1, 10]`。
